    ...
    <a-card :title="title" style="line-height: 40px" class="card">
        ...
        <div class="audio-wrapper">
            <div class="chatList" :style="`width: ${width}; height: ${chartHeight}`">
                <div id="chatScroll" ref="chatContainer" class="chat-scroll"
                    :style="`overflow:${isMove == true ? 'auto' : 'hidden'}`" @mouseenter="mouseenter"
                    @mouseleave="mouseleave">
                    <div v-for="item in chatList" :id="item.id" :key="item.id" class="flex"
                        :class="`${item.isSelf ? 'reverse' : ''}`">
                        <a-avatar v-if="isShowAvatar" class="" :class="item.isSelf ? 'ml-8' : 'mr-8'"
                            :icon="item.isSelf ? 'user' : 'user'" />
                        <span :id="`${'chatData_' + item.id}`" class="chatContent"
                            :class="`${item.isSelf ? 'isColor' : 'noColor'}`">
                            <img v-if="!item.isLoading && item.isPic" :src="item.label" style="width: 100%" />
                            <v-md-preview v-else-if="!item.isLoading && !item.isPic" :text="item.label"
                                class="markdown-body"></v-md-preview>
                            <template v-else>
            ...
            <div v-if="oldFooterStyle" class="footers">
                <a-input v-model.trim="textarea" class="ipt" placeholder="请输入提问内容，按回车进行提问"
                    @pressEnter="onEnter($event.target.value)"></a-input>
                <a-button type="primary" :disabled="sendDisabled" class="sendButton"
                    @click="onEnter(textarea)">发送</a-button>
            </div>
            <a-row v-else class="reportFooter">
                <a-row>
                    <a-textarea v-model.trim="textarea" placeholder="请输入提问内容" :auto-size="{ minRows: 2, maxRows: 6 }"
                        class="reportFooterInput" @pressEnter="pressEnter($event)" />
                </a-row>
                <a-row class="reportFooterButton">
                    <a-col v-if="isShowNetwork" :span="20" class="tx-l">
                        <a-tooltip title="按需搜索网页">
                            <a-button size="small" :class="enableNetworkQuery ? 'netButton' : ''" shape="round" ghost
                    ...
                    <a-col :span="4" class="tx-r">
                        <a-tooltip title="请输入你的问题">
                            <a-button class="sendButton" size="small" type="primary" shape="circle" icon="arrow-up"
                                :disabled="sendDisabled" @click="onEnter(textarea)" />
                            <!-- 发送
              </a-button> -->
                        </a-tooltip>
                    </a-col>
...
@Component
export default class AiChatComponent extends Vue {
    @Prop({ default: 'AI问答' })
    public title: string
        ...
        try {
            ...
            try {
                while (true) {
                    ...
                    const res = String(chunk)
                        .replace(/data:/g, '')
                        .split(this.fetchSplitTxt)
