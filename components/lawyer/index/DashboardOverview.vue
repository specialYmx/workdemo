    ...
    <div class="lawyer-page-header-actions">
      <div class="lawyer-time-range">
        <a-button
          ...
          :key="index"
          :class="['lawyer-time-btn', { active: activeTimeRange === timeOption.value }]"
          @click="handleTimeRangeChange(timeOption.value)"
        >
          {{ timeOption.label }}
        </a-button>
      </div>
      <a-button type="primary" icon="file-pdf" @click="exportReport()"> 导出报告 </a-button>
    </div>
        ...
        <div class="lawyer-chart-section">
          <div class="lawyer-chart-header">
            ...
            <div class="lawyer-chart-actions">
              <a-button type="primary" @click="viewReviews()"> 查看历史记录 </a-button>
            </div>
          </div>
...
<script lang="ts">
  ...
  class DashboardOverview extends Vue {
    ...
    @Prop({ type: [String, Number], default: null }) trendChartValue!: string | number;
    ...
    get trendChartOptions(): EChartOption {
      ...
      return {
        ...
        series: seriesNames.map((name, index) => this.generateSeriesConfig(name, index, index))
      };
    }
