<template>
    <div class="index-page-wrapper">
        <div class="lawyer-page-container">
            <!-- 顶部仪表盘 -->
            <lawyer-index-dashboard-overview :monthly-update-count="monthlyUpdateCount"
                :pending-review-count="pendingReviewCount" :recent-reviews="recentReviews"
                :recent-reviews-loading="listLoading.recentReviews" :trend-chart-data="chartData.trend"
                :trend-chart-loading="chartLoading.trend" :active-time-range="activeTimeRange"
                :trend-chart-period="trendChartPeriod" :trend-chart-value="trendChartValue"
                @time-range-change="handleTimeRangeChange" @trend-period-change="handleTrendPeriodChange"
                @export-report="exportStatisticReport" @view-reviews="goToReviewPageFromHistory" />

            <!-- Top 5 需要人工审核 -->
            <lawyer-index-review-table :reviews="topReviews" :loading="listLoading.topReviews"
                @view-all="goToReviewPageFromPending" @view-detail="viewReviewDetail" @approve="approveReview"
                @reject="rejectReview" />

            <!-- 环形图和来源分布 -->
            <div class="lawyer-two-column-chart-grid">
                <lawyer-index-source-distribution :chart-data="chartData.sources" :legend-items="sourceLegendItems"
                    :loading="chartLoading.sources" />
            </div>

            <!-- 最新法规更新卡片 -->
            <lawyer-index-latest-updates :updates="latestUpdates" :loading="listLoading.latestUpdates"
                @view-all="goToUpdatesPage" @view-detail="viewUpdateDetail" />
        </div>
    </div>
</template>
