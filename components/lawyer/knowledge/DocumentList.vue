    ...
    <div>
        ...
        <div v-if="!loading && documents.length" class="lawyer-document-list">
                    ...
                    <div class="lawyer-document-main-content">
                        ...
                        <div class=" lawyer-document-footer">
                            <div class="lawyer-document-tags">
                                ...
                                <a-tag v-if="doc.appendix" color="cyan">
                                    附录
                                </a-tag>
                            </div>
                            <div class="lawyer-document-actions">
                                <a-button v-for="(action, index) in docActions[doc.id] || []" :key="index"
                                    :type="action.type || 'default'" :class="action.class" @click="action.handler">
                                    {{ action.text }}
                                </a-button>
                            </div>
                        </div>
            ...
            <div v-if="documents.length" class="lawyer-pagination">
                <a-pagination v-bind="paginationConfig" @change="onPageChange" @showSizeChange="onShowSizeChange" />
            </div>
        </div>
    </div>
</template>
...
export default class LawyerKnowledgeDocumentList extends Vue {
    ...
    onDocumentAction(action: string, doc: KnowledgeDataItem): { action: string; doc: KnowledgeDataItem } {
        return { action, doc }
    }
