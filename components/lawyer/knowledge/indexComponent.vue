...
export default class LawyerKnowledgeIndexComponent extends Vue {
  ...
  get uploadModalTitle(): string {
    return this.isAddMode
      ? "新增文档"
      : `更新文档: ${this.currentUploadDocTitle}`;
  }
  ...
  async collectDocument(doc: KnowledgeDataItem): Promise<void> {
    ...
    try {
      ...
      if (success) {
        ...
        const docIndex = this.allDocuments.findIndex(
          (item) => item.id === doc.id
        );
        if (docIndex !== -1) {
      ...
      onOk: async () => {
        try {
          ...
          const shouldGoToPreviousPage =
            isLastItemOnPage && this.currentPage > 1;
