    ...
    <div class="lawyer-search-bar">
      <div class="lawyer-search-box">
        ...
        <a-button
          ...
          @click="clearSearch"
        />
        <a-button type="primary" icon="search" class="lawyer-search-button" @click="onSearch" />
      </div>
...
<script lang="ts">
  ...
  class DivTextSearch extends Vue {
    ...
    beforeDestroy() {
      // 取消防抖函数
      if (this.debouncedHighlight && typeof this.debouncedHighlight.cancel === 'function') {
    ...
    clearHighlights() {
      const content = this.$refs.searchableContent as HTMLElement;
      const highlights = content.querySelectorAll('.lawyer-highlight, .lawyer-current-highlight');
    ...
    updateCurrentHighlight() {
      this.highlightSpans.forEach(span => span.classList.remove('lawyer-current-highlight'));

      if (this.highlightSpans.length === 0) {
        return;
      }

      if (this.currentIndex < 0 || this.currentIndex >= this.highlightSpans.length) {
        this.currentIndex = 0;
      }
