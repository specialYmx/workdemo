      ...
      <div class="ai-chat">
        <div ref="aiMessages" class="ai-messages">
          <div
            ...
            :key="index"
            :class="['ai-message', msg.isUser ? 'ai-message-user' : 'ai-message-ai']"
          >
            <div>
              <div v-if="msg.isUser">
                {{ msg.content }}
          ...
          <div class="custom-input-container">
            ...
            <div class="input-toolbar">
              ...
              <div class="toolbar-left">
                <a-tooltip :title="enableNetworkQuery ? '' : '点击开启联网搜索'" placement="top">
                  <a-button
                    :type="enableNetworkQuery ? 'primary' : 'default'"
                    size="small"
                    :class="['network-btn', { 'network-btn-active': enableNetworkQuery }]"
              ...
              <div class="toolbar-right">
                ...
                <a-button v-else type="danger" size="small" class="stop-btn" @click="handleStop">
                  <a-icon type="stop" />
                  停止
                </a-button>
              </div>
...
<script lang="ts">
  ...
  export default class DocumentAIChat extends Vue {
    ...
    async askAi(question: string): Promise<void> {
      if (!question.trim() || this.aiLoading) {
        return;
      }
      ...
      try {
        ...
        // 使用fetch进行流式请求
        const response: Response = await fetch(`${baseURL}${api.lawyer.getAIRobotAnswer}`, {
          method: 'POST',
          headers: {
            userId,
    ...
    // 处理流式响应
    async processStreamResponse(response: Response, aiMessage: AiMessage): Promise<void> {
    ...
    formatTextWithNewlines(text: string): string {
      ...
      const pattern = /([一二三四五六七八九十]+、|（[一二三四五六七八九十]+）|\d+[.、])/g;
