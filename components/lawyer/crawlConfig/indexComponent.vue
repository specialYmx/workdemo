      ...
      <a-card class="lawyer-table-card" :bordered="false">
        ...
        <div class="lawyer-search-form">
          <a-row :gutter="16">
            <a-col :span="4">
              <a-input
                v-model="searchParams.websiteCode"
                placeholder="网站代码"
                allow-clear
          ...
          <template slot="keywords" slot-scope="text">
            <div v-if="text && text.length > 0" class="lawyer-keywords-container">
              ...
              <a-tag
                ...
                :title="`还有${text.length - 3}个关键词：${text.slice(3).join('、')}`"
              >
                +{{ text.length - 3 }}个
              </a-tag>
            </div>
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            ...
            <a-form-model-item label="爬取起始时间" prop="crawlStartDate">
              <a-date-picker
                v-model="formData.crawlStartDate"
                placeholder="请选择爬取起始时间"
                style="width: 100%"
                show-time
...
<script lang="ts">
  ...
  export default class CrawlConfigIndexComponent extends Vue {
    ...
    formRules = {
      ...
      websiteCode: [{ required: true, message: '请输入网站代码', trigger: 'blur' }],
      websiteUrl: [{ required: true, message: '请输入网站URL', trigger: 'blur' }],
      crawlStartDate: [{ required: true, message: '请选择爬取起始时间', trigger: 'change' }],
      maxPageLimit: [
    ...
    async handleModalOk(): Promise<void> {
      try {
        ...
        if (this.editingRecord) {
          ...
          // 新增
          response = await this.$roadLawyerService.addCrawlConfig(params as AddCrawlConfigParams);
        }
